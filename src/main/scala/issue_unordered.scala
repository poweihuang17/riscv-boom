   ...
   extends IssueUnit(num_issue_slots, issue_width, num_wakeup_ports)
{
   ...
   val entry_wen_oh = Seq.fill(num_issue_slots)(Wire(Bits(width=DISPATCH_WIDTH)))
   ...
   var allocated = Seq.fill(DISPATCH_WIDTH)(Wire(Bool(false))) // did an instruction find an issue width?


   for (i <- 0 until num_issue_slots)
   {
      var next_allocated = Seq.fill(DISPATCH_WIDTH)(Wire(Bool()))
   ...
   for (i <- 0 until num_issue_slots)
   {
      val temp_uop_val = Seq.fill(DISPATCH_WIDTH)(Wire(Bool()))
      ...
      entry_wen_oh(i) := Cat(temp_uop_val.reverse)
   }
