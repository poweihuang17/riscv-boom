...
class FetchUnit(fetch_width: Int)(implicit p: Parameters) extends BoomModule()(p)
{
   ...
   val fseq_reg = Reg(init = 0.U(xLen.W))
         ...
         for (i <- 0 until fetch_width)
         {
            when (fetch_bundle.mask(i))
            {
               ...
               printf("%d; O3PipeView:fetch:%d:0x%x:0:%d:DASM(%x)\n",
                  ...
                  (fetch_bundle.pc.asSInt() & (-(fetch_width*coreInstBytes)).S).asUInt() + (i << 2).U,
                  fetch_bundle.debug_events(i).fetch_seq,
   ...
   io.imem.btb_update.bits.isReturn   := Mux(br_unit.btb_update_valid, br_unit.btb_update.isReturn, false.B)
   io.imem.btb_update.bits.isValid    := Mux(br_unit.btb_update_valid, true.B, io.bp2_is_cfi)
