...
class MicroOp(implicit p: Parameters) extends BoomBundle()(p)
{
   ...
   val iw_state         = UInt(2.W)             // what is the next state of this uop in the issue window? useful
   ...
   val wakeup_delay     = UInt(log2Up(MAX_WAKEUP_DELAY).W) // unused
   ...
   val fetch_pc_lob     = UInt(log2Up(FETCH_WIDTH*coreInstBytes).W) // track which PC was used to fetch this
                                                                          // instruction
   ...
   val mem_cmd          = UInt(4.W)             // sync primitives/cache flushes
   val mem_typ          = UInt(3.W)             // memory mask type for loads/stores
   val is_fence         = Bool()
   val is_fencei        = Bool()
...
class CtrlSignals extends Bundle()
{
   ...
   val op_fcn      = UInt(rocket.ALU.SZ_ALU_FN.W)
   val fcn_dw      = Bool()
   val rf_wen      = Bool()
