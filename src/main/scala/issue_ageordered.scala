   ...
   extends IssueUnit(num_issue_slots, issue_width, num_wakeup_ports)
{
   ...
   val vacants = issue_slots.map(s => !(s.io.valid)) ++ io.dis_valids.map(!_.toBool)
   ...
   val will_be_valid = (0 until num_issue_slots).map(i => issue_slots(i).io.will_be_valid) ++
                       (0 until DISPATCH_WIDTH).map(i => io.dis_valids(i) &&
                                                         !io.dis_uops(i).exception &&
                                                         !io.dis_uops(i).is_fence &&
                                                         !io.dis_uops(i).is_fencei)

   val uops = issue_slots.map(s=>s.io.updated_uop) ++ dis_uops.map(s=>s)
   for (i <- 0 until num_issue_slots)
   {
      issue_slots(i).io.in_uop.valid := false.B
      issue_slots(i).io.in_uop.bits  := uops(i+1)
      for (j <- 1 to MAX_SHIFT by 1)
      {
         when (shamts_oh(i+j) === (1 << (j-1)).U)
         {
            issue_slots(i).io.in_uop.valid := will_be_valid(i+j)
            issue_slots(i).io.in_uop.bits  := uops(i+j)
         }
      }
      issue_slots(i).io.wakeup_dsts  := io.wakeup_pdsts
      issue_slots(i).io.brinfo       := io.brinfo
      issue_slots(i).io.kill         := io.flush_pipeline
      issue_slots(i).io.clear        := shamts_oh(i) =/= 0.U
   }
   ...
   val will_be_available = (0 until num_issue_slots).map(i =>
                              (!issue_slots(i).io.will_be_valid || issue_slots(i).io.clear) && !(issue_slots(i).io.in_uop.valid))
   ...
   val requests = issue_slots.map(s => s.io.request)
   ...
   for (i <- 0 until num_issue_slots)
   {
      issue_slots(i).io.grant := false.B
